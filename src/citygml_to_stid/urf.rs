...
pub struct UrfStorage {
    pub count: i32,
    pub urf_info: UrfInfo,
}

pub fn urf_info(num_of_parallel: usize) -> Result<Option<UrfInfo>, Box<dyn Error>> {
...
fn process_one_file(file_path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let mut storage: Vec<UrfStorage> = Vec::new();
    let mut buf = Vec::<u8>::new();
    let mut in_district = false;

    let mut district_count = 0;
    let mut urfinfo = UrfInfo {
    ...
    loop {
        ...
        match ev {
            Event::Start(e) => {
                ...
                if tag_name.as_slice() == b"urf:FirePreventionDistrict" && !in_district {
                    in_district = true;
                    for a in &attrs {
                        if a.key.as_ref() == b"gml:id" {
                            urfinfo.district_id = a.unescape_value()?.to_string();
                        }
                    }
                }
            ...
            Event::End(e) => {
                ...
                if in_district && tag_name == b"urf:FirePreventionDistrict" {
                    println!("urf fire prevention district count : {}", district_count);
                    storage.push(UrfStorage {
                        count: district_count,
                        urf_info: urfinfo.clone(),
                    });
                    district_count += 1;
                    in_district = false;
    ...
    file::save_urf_infos_json(storage, format!("{}_stid", file_path.display()))?;
    Ok(())
}
