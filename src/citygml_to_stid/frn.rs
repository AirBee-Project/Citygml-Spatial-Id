...
pub struct FrnStorage {
    pub count: i32,
    pub frn_info: FrnInfo,
}

pub fn frn_info(num_of_parallel: usize) -> Result<Option<FrnInfo>, Box<dyn Error>> {
...
fn process_one_file(file_path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let mut storage: Vec<FrnStorage> = Vec::new();
    let mut buf = Vec::<u8>::new();
    let mut in_furniture = false;
    let mut current_tag: Option<Vec<u8>> = None;

    let mut furniture_count = 0;
    let mut frninfo = FrnInfo {
        furniture_id: String::new(),
    ...
    loop {
        ...
        match ev {
            Event::Start(e) => {
                ...
                if tag_name.as_slice() == b"frn:CityFurniture" && !in_furniture {
                    in_furniture = true;
                    for a in &attrs {
                        if a.key.as_ref() == b"gml:id" {
                            frninfo.furniture_id = a.unescape_value()?.to_string();
                        }
                    }
                }
            ...
            Event::End(e) => {
                ...
                if in_furniture && tag_name == b"frn:CityFurniture" {
                    println!("frn furniture count : {}", furniture_count);
                    storage.push(FrnStorage {
                        count: furniture_count,
                        frn_info: frninfo.clone(),
                    });
                    furniture_count += 1;
                    in_furniture = false;
    ...
    file::save_frn_infos_json(storage, format!("{}_stid", file_path.display()))?;
    Ok(())
}
