...
pub struct HtdStorage {
    pub count: i32,
    pub htd_info: HtdInfo,
}
...
pub fn htd_info(num_of_parallel: usize) -> Result<Option<HtdInfo>, Box<dyn Error>> {
...
fn process_one_file(file_path: &Path) -> Result<(), Box<dyn Error>> {
        ...
        match ev {
                ...
                if tag_name.as_slice() == b"wtr:WaterBody" && !in_waterbody {
                    ...
                    for a in &attrs {
                        if a.key.as_ref() == b"gml:id" {
                            htdinfo.waterbody_id = a.unescape_value()?.to_string();
                        }
                    }
                }
            ...
            Event::End(e) => {
                ...
                if in_waterbody && tag_name == b"wtr:WaterBody" {
                    println!("htd waterbody count : {}", waterbody_count);
                    storage.push(HtdStorage {
                        count: waterbody_count,
                        htd_info: htdinfo.clone(),
                    });
                    waterbody_count += 1;
                    in_waterbody = false;
    ...
    file::save_htd_infos_json(storage, format!("{}_stid", file_path.display()))?;
    Ok(())
}
