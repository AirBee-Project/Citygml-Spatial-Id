...
pub struct BridStorage {
    pub count: i32,
    pub bridge_info: BridgeInfo,
}

pub fn brid_info(num_of_parallel: usize) -> Result<Option<BridgeInfo>, Box<dyn Error>> {
...
fn process_one_file(file_path: &Path) -> Result<(), Box<dyn Error>> {
    ...
    let mut storage: Vec<BridStorage> = Vec::new();
    let mut buf = Vec::<u8>::new();
    let mut in_bridge = false;

    let mut bridge_count = 0;
    let mut bridgeinfo = BridgeInfo {
    ...
    loop {
        ...
        match ev {
            Event::Start(e) => {
                ...
                if tag_name.as_slice() == b"brid:Bridge" && !in_bridge {
                    in_bridge = true;
                    for a in &attrs {
                        if a.key.as_ref() == b"gml:id" {
                            bridgeinfo.bridge_id = a.unescape_value()?.to_string();
                        }
                    }
                }
            ...
            Event::Text(t) => {
                if in_bridge {
                    ...
                    if let Some(mapped) = codespace_ctx.resolve_text(&text_val, &mut code_space_cache)? {
                        ...
                        if tag_name.as_slice() == b"gml:posList" {
                            ...
                            bridgeinfo.stid_set.extend(ids);
                        }
                    }
                }
            }
            Event::End(e) => {
                ...
                if in_bridge && tag_name == b"brid:Bridge" {
                    println!("bridge count : {}", bridge_count);
                    storage.push(BridStorage {
                        count: bridge_count,
                        bridge_info: bridgeinfo.clone(),
                    });
                    bridge_count += 1;
                    in_bridge = false;
    ...
    file::save_bridge_infos_json(storage, format!("{}_stid", file_path.display()))?;
